...
// 🚀 التعديل: إزالة unique من roomNumber عشان نقدر نكرره في أقسام مختلفة
const bedSchema = new mongoose.Schema({ bedNumber: { type: String, required: true }, isOccupied: { type: Boolean, default: false } });
const roomSchema = new mongoose.Schema({ roomNumber: { type: String, required: true }, floor: { type: String, required: true }, totalBeds: { type: Number, required: true, min: 1 }, beds: [bedSchema] }, { timestamps: true });
// إنشاء Index مركب عشان يمنع تكرار نفس الغرفة في نفس القسم فقط
...
const admin = (req, res, next) => {
  ...
  else { res.status(403).json({ error: 'غير مصرح لك، هذه الصلاحية للمديرين فقط' }); }
};

const generateToken = (id) => { return jwt.sign({ id }, JWT_SECRET, { expiresIn: '30d' }); };

// ==========================================
// 4. المسارات والعمليات (Routes)
// ==========================================
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    if (user && (await bcrypt.compare(password, user.password))) {
      const token = generateToken(user._id); user.token = token; await user.save();
...
app.put('/api/users/theme', protect, async (req, res) => {
  try { req.user.theme = req.body.theme; await req.user.save(); res.json({ message: 'تم التحديث' }); } 
  catch (error) { res.status(500).json({ error: error.message }); }
});
...
app.put('/api/users/:id', protect, admin, async (req, res) => {
  try {
    ...
    if (password && password.trim() !== '') { const salt = await bcrypt.genSalt(10); user.password = await bcrypt.hash(password, salt); user.token = null; } 
    await user.save();
    res.json({ _id: user._id, name: user.name, username: user.username, role: user.role });
  } catch (error) { res.status(500).json({ error: error.message }); }
});

// 🚀 التعديل: مسار الحذف مع منع حذف النفس
app.delete('/api/users/:id', protect, admin, async (req, res) => {
  try {
    if (req.params.id === req.user._id.toString()) return res.status(400).json({ error: 'لا يمكنك حذف حسابك الشخصي' });
    await User.findByIdAndDelete(req.params.id);
    res.json({ message: 'تم حذف المستخدم نهائياً' });
  } catch (error) { res.status(500).json({ error: error.message }); }
});

// 🚀 التعديل: منع طرد النفس
app.post('/api/users/:id/logout', protect, admin, async (req, res) => {
  try {
    if (req.params.id === req.user._id.toString()) return res.status(400).json({ error: 'لا يمكنك طرد نفسك، استخدم زر تسجيل الخروج العادي' });
    const user = await User.findById(req.params.id);
    if (!user) return res.status(404).json({ error: 'المستخدم غير موجود' });
...
app.post('/api/rooms', protect, async (req, res) => { 
  try { 
    ...
    // 🚀 التعديل: التحقق اليدوي إن الغرفة دي مش موجودة في نفس القسم
    const roomExists = await Room.findOne({ roomNumber, floor });
    if (roomExists) return res.status(400).json({ error: 'الغرفة دي مسجلة بالفعل في نفس القسم' });

    const beds = []; for (let i = 1; i <= totalBeds; i++) { beds.push({ bedNumber: `${roomNumber}-${i}` }); } 
    const room = new Room({ roomNumber, floor, totalBeds, beds }); 
    await room.save(); 
    res.status(201).json(room); 
  } catch (error) { 
    if(error.code === 11000) return res.status(400).json({ error: 'الغرفة دي مسجلة بالفعل في نفس القسم' });
...
app.put('/api/rooms/:id', protect, async (req, res) => { 
  try { 
    ...
    // 🚀 التعديل: التحقق عند التعديل
    const roomExists = await Room.findOne({ roomNumber, floor });
    if (roomExists && roomExists._id.toString() !== req.params.id) {
      return res.status(400).json({ error: 'الغرفة دي مسجلة بالفعل في نفس القسم' });
    }

    const room = await Room.findById(req.params.id); 
    if (!room) return res.status(404).json({ error: 'الغرفة غير موجودة' }); 
    room.roomNumber = roomNumber || room.roomNumber; room.floor = floor || room.floor; 
    if (totalBeds && totalBeds > room.beds.length) { const diff = totalBeds - room.beds.length; for (let i = 1; i <= diff; i++) { room.beds.push({ bedNumber: `${room.roomNumber}-${room.beds.length + 1}` }); } room.totalBeds = totalBeds; } 
    await room.save(); 
    res.json(room); 
  } catch (error) { 
    if(error.code === 11000) return res.status(400).json({ error: 'الغرفة دي مسجلة بالفعل في نفس القسم' });
...
// ----- العلامات الحيوية، الأدوية، الملاحظات، المهام والتقارير -----
// (باقي الأكواد زي ما هي بدون تغيير عشان منستهلكش مساحة على الفاضي، وهي شغالة 100%)
